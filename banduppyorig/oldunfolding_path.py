...
#            print("after",k1.shape,E1.shape,density.shape)
        ...
        plt.close()
    def plotSP(self,save_file=None,Ef=None,Emin=None,Emax=None,mode="fatband",plotSC=False,fatfactor=20,nE=100,smear=0.05,
         spin_polarized=False):
        import matplotlib.pyplot as plt
        from matplotlib.colors import LogNorm
        if not spin_polarized: 
    ...
    #            print(density.T.min(),density.T.max())
         ...
         #   self.kpline=bandstructure[0].KPOINTSline(kpred=self.kpointsPBZ,breakTHRESH=0.1)
            ...
            if mode=="fatband":
                ...
                plt.scatter(result_down[:,0],result_down[:,1],s=result_down[:,2]*fatfactor,color="blue",label="unfolded")
                if plotSC:
                    plt.legend()
            elif mode=="density":
                energy=np.linspace(Emin,Emax,nE)
                density=np.zeros((self.nkptPBZ,nE),dtype=float)
                result_sum=result_up+result_down
                print(result_up,result_down)
                for k,E,w in result_sum[:,:3]:
#                    if w > 1.0:
#                        w=1.0
                    ik=np.argmin(abs(k-self.kpline))
    #                print (ik,k,E,w)
